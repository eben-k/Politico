...
pool.query('CREATE TABLE IF NOT EXISTS users(id int GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, firstname VARCHAR(100) not null, lastname VARCHAR(100) not null, email VARCHAR(100) UNIQUE not null, phoneNumber VARCHAR(100) not null, passportUrl VARCHAR(50) not null, isAdmin Boolean not null DEFAULT (false), password VARCHAR(250) not null)');
pool.query('CREATE TABLE IF NOT EXISTS parties(id SERIAL PRIMARY KEY, name VARCHAR(100) not null, hqAddress VARCHAR(250) not null, logoUrl VARCHAR(100) not null, email VARCHAR(100), phoneNumber VARCHAR(100), created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)');
